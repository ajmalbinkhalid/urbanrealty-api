...
import type { UserRequest as AgencyRequest } from "@/types/user-type";
...
class AppUserCmsControllerClass {
  async getPrivacyPolicy(req: AgencyRequest, res: Response): Promise<void> {
    try {
  ...
  async getTermsAndConditions(req: AgencyRequest, res: Response): Promise<void> {
    try {
      const cms = await CmsModel.findOne({ deletedAt: null }).lean();
  ...
  async getFaq(req: AgencyRequest, res: Response): Promise<void> {
    try {
      const cms = await CmsModel.findOne({ deletedAt: null }).lean();
      if (!cms?.faq) {
  ...
  async getHowItWorks(req: AgencyRequest, res: Response): Promise<void> {
    try {
      const cms = await CmsModel.findOne({ deletedAt: null }).lean();
      if (!cms?.howItWorks) {
...
export const AppAgencyCmsController = new AppUserCmsControllerClass();
