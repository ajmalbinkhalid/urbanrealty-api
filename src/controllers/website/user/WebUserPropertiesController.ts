...
} from "@/routes/website/user/validations/UserPropertyRouterValidation";
...
class WebUserPropertiesControllerClass {
  async createProperty(req: UserRequest, res: Response): Promise<void> {
    ...
    try {
      ...
      return ResJson.success(res, isDraft ? "Property saved as draft" : "Property created successfully", { property: result.property });
    } catch (error) {
      await session.abortTransaction();
  ...
  async getUserProperties(req: UserRequest, res: Response): Promise<void> {
    try {
      ...
      const sort = { createdAt: sortOrder } as { createdAt: 1 | -1 };
      ...
      const properties = await DBHelper.fetch({
        model: PropertyModel,
        req,
        searchFields: ["propertyInformation.title.en", "propertyInformation.title.en"],
        filters: {
          "owner.ownerType": OwnerTypeEnum.user,
          "owner.ownerId": new mongoose.Types.ObjectId(userId),
      ...
      return ResJson.success(res, "User properties fetched successfully", {
        properties,
      });
    } catch (error) {
      return ResJson.error(res, error);
  ...
  async getUserPropertyDetails(req: UserRequest, res: Response): Promise<void> {
    try {
      const userId = req.user.userId;
      ...
      const propertyDetails = {
        ...
        propertyInformation: {
          ...
          titleObject: {
            en: property.propertyInformation.title.en,
            ar: property.propertyInformation.title.ar,
          },
          description: property.propertyInformation.description[ReqHelpers.locale(req)],
          descriptionObject: {
            en: property.propertyInformation.description.en,
            ar: property.propertyInformation.description.ar,
          },
          landmark: property.propertyInformation.landmark[ReqHelpers.locale(req)],
          landmarkObject: {
            en: property.propertyInformation.landmark.en,
            ar: property.propertyInformation.landmark.ar,
          },
        ...
        coverImage: property.coverImage && FileHelper.getUrl(property.coverImage),
        galleryImages: property.galleryImages ? property.galleryImages.map((img) => FileHelper.getUrl(img)) : [],
...
export const WebUserPropertiesController = new WebUserPropertiesControllerClass();
