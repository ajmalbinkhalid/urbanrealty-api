...
import { PropertyModel, getPropertyOwnerDetails } from "@/database/models/PropertyModel";
import { SubCategoryModel } from "@/database/models/SubCategoryModel";
import type { TToggleFavoriteParams } from "@/routes/website/user/validations/UserFavoriteRouterValidation";
...
class WebUserFavoriteControllerClass {
  ...
  async getFavoriteProperties(req: UserRequest, res: Response): Promise<void> {
    try {
      ...
      return ResJson.success(res, "Favorite properties fetched successfully", enrichedResult);
    } catch (error) {
      return ResJson.error(res, error);
    }
  }
...
export const WebUserFavoriteController = new WebUserFavoriteControllerClass();
