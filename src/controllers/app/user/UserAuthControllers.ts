...
import type { TAppUserDeleteNotificationRequest, TAppUserRegisterRequest, TAppUserRequestOtpRequest, TAppUserSaveNotificationRequest, TAppUserUpdateProfileRequest } from "@/routes/app/user/validations/UserAuthRouterValidation";
...
class AppUserAuthControllerClass {
  ...
  async userRegister(req: Request, res: Response): Promise<void> {
    try {
      const { firstName,phone, lastName, email } = req.body as TAppUserRegisterRequest;
      ...
      const identifierData: TIdentifierData = {
        ...
        source: OTPSourceEnum.userApp,
      };

      logger.info(`Registering user with identifier: ${identifierData.identifier} (${identifierData.identifierType})`);
  ...
  async userRequestOtp(req: Request, res: Response): Promise<void> {
    try {
      const { email } = req.body as TAppUserRequestOtpRequest;
      ...
      if (identifierData === null) {
        return ResJson.invalid(res, "Email must be provided.");
      }
    ...
    try {
      ...
      if (tokenType === "register" && userData) {
        ...
        ResJson.success(res, "User registered successfully", {
          accessToken,
          expiresIn,
          user: AppUserAuthControllerClass.getUser(user[0]),
        });
      ...
      if (tokenType === "login") {
        ...
        if (!user) {
          ResJson.unauthenticated(res, "User not found or not verified");
          return;
        }
        ...
        const properties = await AppUserAuthControllerClass.getUserProperties(user._id);

        ResJson.success(res, "OTP verified successfully", {
          accessToken,
          expiresIn,
          user: AppUserAuthControllerClass.getUser(user, properties),
        });
  ...
  async updateProfile(req: UserRequest, res: Response): Promise<void> {
    try {
      const { userId } = req.user;
      const { firstName, lastName, logo } = req.body as TAppUserUpdateProfileRequest;
  ...
  async saveNotificationToken(req: UserRequest, res: Response): Promise<void> {
    try {
      const { userId } = req.user;
  ...
  async deleteNotificationToken(req: UserRequest, res: Response): Promise<void> {
    try {
      const { userId } = req.user;
...
export const AppUserAuthController = new AppUserAuthControllerClass();
