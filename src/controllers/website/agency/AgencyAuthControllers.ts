...
import type { TWebAgencyRegisterRequest, TWebAgencyRequestOtpRequest, TWebAgencyUpdateProfileRequest } from "@/routes/website/agency/validations/AgencyAuthRouterValidation";
...
class WebAgencyAuthControllerClass {
  static getAgency(agency: TAgencyModel): Record<string, unknown> {
    return {
      ...
      activeRentPropertiesCount: agency.activeRentPropertiesCount,
      activeSalePropertiesCount: agency.activeSalePropertiesCount,
      about: agency.about,
  ...
  async agencyRegister(req: Request, res: Response): Promise<void> {
    try {
      const { firstName, lastName, email, company, phone, cRNumber } = req.body as TWebAgencyRegisterRequest;
      ...
      const identifierData: TIdentifierData = {
        ...
        source: OTPSourceEnum.agencyWeb,
      };
      ...
      ResJson.success(res, `Agency registered. OTP sent to ${identifierData.identifierType}. ${otp}`, {
        sessionToken,
        expiresAt,
  ...
  async agencyRequestOtp(req: Request, res: Response): Promise<void> {
    try {
      const { email } = req.body as TWebAgencyRequestOtpRequest;
      ...
      if (!existingAgencyTeam) {
        ResJson.notFound(res, "Agency team member not found or not verified");
        return;
      }
    ...
    try {
      ...
      if (tokenType === "login") {
        ...
        ResJson.success(res, "OTP verified successfully", {
          accessToken,
          expiresIn,
          agency: WebAgencyAuthControllerClass.getAgency(agency),
          agencyTeam: WebAgencyAuthControllerClass.getAgencyTeamMember(agencyTeam),
        });
  ...
  async getAgencyProfile(req: AgencyRequest, res: Response): Promise<void> {
    try {
      ...
      const activePropertiesCount = await PropertyModel.countDocuments({
      ...
      ResJson.success(res, "Agency profile retrieved successfully", {
        agency: {
          ...WebAgencyAuthControllerClass.getAgency(agency),
  ...
  async updateAgencyProfile(req: AgencyRequest, res: Response): Promise<void> {
    ...
    try {
      ...
      const { companyLogo, companyName, cRNumber, companyEmail, companyPhone, companyWhatsapp, about, firstName, lastName } = req.body as TWebAgencyUpdateProfileRequest;
      ...
      ResJson.success(res, "Profile updated successfully", {
        agency: WebAgencyAuthControllerClass.getAgency(updatedAgency),
        agencyTeam: WebAgencyAuthControllerClass.getAgencyTeamMember(updatedAgencyTeam),
      });
    } catch (error) {
      await session.abortTransaction();
      session.endSession();
...
export const WebAgencyAuthController = new WebAgencyAuthControllerClass();
